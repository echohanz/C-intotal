...
private:
    ...
    void DFTr(int vertex,std::map<int,bool> &history) const;//why have to const????

    struct DJK{
        int vertex;
        bool k; //
        double d;  // -1 is infinite
        int p;  // -1 is none, -2 is unknown
    };
...
template <typename VertexInfo, typename EdgeInfo>
void Digraph<VertexInfo, EdgeInfo>::DFTr(int vertex,std::map<int,bool> &history) const
{
    history[vertex] = true;
    for (auto k : map.at(vertex).edges){
        if (!history[k.toVertex]){DFTr(k.toVertex,history);}
    }
}
...
std::vector<std::pair<int, int>> Digraph<VertexInfo, EdgeInfo>::edges() const
{
    ...
    for (auto &i : map){
        for (auto &k : i.second.edges){
            edgelist.push_back(std::pair<int,int>(k.fromVertex,k.toVertex));
        }
    }
    return edgelist;
}
...
{
    std::vector<std::pair<int, int>> edgelist;
    auto verptr = map.find(vertex);
    if (verptr == map.end()){ throw DigraphException{"vertex does not exist."};}
    for (auto &k : verptr->second.edges){
        if (k.fromVertex == vertex){ edgelist.push_back(std::pair<int,int>(k.fromVertex,k.toVertex)); }
    }
    return edgelist;
}
...
{
    auto keyptr = map.find(vertex);
    if (keyptr == map.end()){ throw DigraphException{"vertex does not exist."};}
    return keyptr->second.vinfo;
}
...
EdgeInfo Digraph<VertexInfo, EdgeInfo>::edgeInfo(int fromVertex, int toVertex) const
{
    for (auto &i : map){
        for (auto &k : i.second.edges){
            if (k.fromVertex == fromVertex && k.toVertex == toVertex){return k.einfo;}
        }
    }
        throw DigraphException{"Such edge does not exist."};
}
...
{
    auto keyptr = map.find(vertex);
    if (keyptr == map.end())//when there is no such vertex
    {
        map[vertex] = DigraphVertex<VertexInfo, EdgeInfo>{vinfo};
    }  
    else {
        throw DigraphException{"Vertex already existed."};
    }
}
...
{
    auto keyptr = map.find(fromVertex);
    if (keyptr != map.end())//when there is such vertex
    {
        for (auto &k : keyptr->second.edges){
            if (k.toVertex == toVertex){ throw DigraphException{"Edge already existed."};}
        }
        keyptr->second.edges.push_back(DigraphEdge<EdgeInfo>{fromVertex,toVertex,einfo});
    }  
    else {
...
{
    auto keyptr = map.find(vertex);
    if (keyptr == map.end()){throw DigraphException{"Vertex does not exist already."};}
    map.erase(keyptr);
}
...
{
    auto keyptr = map.find(fromVertex);
    if (keyptr == map.end()){throw DigraphException{"that fromVertex does not exist."};}
    for (typename std::list<DigraphEdge<EdgeInfo>>::iterator k = map[fromVertex].edges.begin(); k != map[fromVertex].edges.end();k++){
         if (k->toVertex == toVertex){map[fromVertex].edges.erase(k);return;}
    }
...
{
    auto keyptr = map.find(vertex);
    if (keyptr == map.end()){throw DigraphException{"that Vertex does not exist."};}
    return keyptr->second.edges.size();
}
...
{
    std::map<int,bool> mytravel;
    //std::vector<bool> mytravel;
    for (auto k: map)
    {
        //mytravel.clear();
        //for (int i=0;i<map.size();i++){mytravel.push_back(false);}// defult set
        for (auto m: map){mytravel[m.first] = false;}
        //std::cout<< k.first <<std::endl;     
        
        DFTr(k.first,mytravel);
        for (auto m: map){ if (!mytravel[m.first]){return false;}}
    }
    return true;
}
    ...
    std::function<double(const EdgeInfo&)> edgeWeightFunc) const
{
    ...
    std::map<int,Digraph<VertexInfo, EdgeInfo>::DJK> table;

    //init table
    for (auto &i: map)
    {
        table[i.first] = Digraph<VertexInfo, EdgeInfo>::DJK{i.first,false,-1,-2};
    }

    // init start point
    table[startVertex].d = 0;
    table[startVertex].p = -1;

    //DJK algorithem
    std::priority_queue<std::pair<int,int>, std::vector<std::pair<int,int>>, std::greater<std::pair<int,int>>> giveSmallest;// pair(rank,vertex)
    giveSmallest.push(std::pair<int,int>(0,startVertex));

    while(!giveSmallest.empty())
    {
        int ver = giveSmallest.top().second; //dequeuue
        giveSmallest.pop();
        if (table[ver].k == false)
        {
            table[ver].k = true;
            for (auto &v :map.at(ver).edges)
            {
                int dest = v.toVertex;
                if (table[dest].d > (edgeWeightFunc(edgeInfo(ver,dest)) + table[ver].d) || table[dest].d < 0)
                { 
                    table[dest].d = edgeWeightFunc(edgeInfo(ver,dest)) + table[ver].d;
                    table[dest].p = ver;
                    giveSmallest.push(std::pair<int,int>(table[dest].d,dest));
                }
            }
        }
    }
    ...
    //return format
    for (auto &i : table){
        if (i.second.p == -1){ route[i.first] = i.first; }
        else {route[i.first] = i.second.p;}
    }
